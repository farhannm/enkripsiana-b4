...
void buatfilekedirectory(const char* filename, const char* buffer) {
    char filePath[256];
    const char* inputDirectory = "Crypto"; // Current directory

    snprintf(filePath, sizeof(filePath), "%s/%s.txt", inputDirectory, filename);
    ...
    printf("File '%s' berhasil dibuat dengan isi:\n%s\n", filename, buffer);
}
...
int BuatFile() {
    ...
    printf("Masukkan teks yang ingin disimpan dalam file:\n");
    scanf(" %[^\n]", buffer);

    buatfilekedirectory(fileName, buffer);
}
